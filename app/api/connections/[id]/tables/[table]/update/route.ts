...
type UpdateBody = {
  ...
  data: Record<string, unknown>;   // değişecek alanlar
};
...
export async function POST(
  ...
  if (!body?.pk || typeof body.pk !== "object" || !body?.data || typeof body.data !== "object") {
    return NextResponse.json({ error: "Missing pk/data" }, { status: 400 });
  }
  ...
  // kolon adlarını güvenceye al (enjekte edilmesin)
  const badKey = (k: string) => !/^[A-Za-z0-9_\-$]+$/.test(k);

  if (pkKeys.some(badKey)) {
  ...
  if (dataKeys.some(badKey)) {
    return NextResponse.json({ error: "Invalid data column name" }, { status: 400 });
  }
  ...
  try {
    ...
    // SET kısmı
    const setSql = dataKeys.map((k) => `\`${schema}\`.\`${tbl}\`.\`${k}\` = ?`).join(", ");
    const setParams = dataKeys.map((k) => (body.data as any)[k]);
    ...
    const [res] = await db.execute(
      `UPDATE \`${schema}\`.\`${tbl}\` SET ${setSql} WHERE ${whereSql} LIMIT 1`,
    ...
    return NextResponse.json({ error: "Update failed", detail: String(e?.message || e) }, { status: 502 });
  } finally {
